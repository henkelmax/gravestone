...
public class GraveProcessor {
	...
	public boolean placeGraveStone(List<EntityItem> drops) {
		...
		try {
			...
			this.gravePosition=deathPosition;
			Log.i("Grave from '" +entity.getDisplayName().getUnformattedText() +"' cant be created (No space)");
			return false;
		}
		...
		try {
			...
			graveTileEntity.setPlayerName(entity.getDisplayName().getUnformattedText());
	...
	public void givePlayerNote(){
		...
		DeathInfo info=new DeathInfo(gravePosition, player.dimension, items, player.getDisplayNameString(), time, player.getUniqueID());
